...
// GET: 결제 설정 조회
export const GET = withAuth({ auth: 'admin', rateLimit: 'default' }, async () => {
...
// PUT: 결제 설정 업데이트
export const PUT = withAuth({ auth: 'admin', rateLimit: 'default' }, async (request: NextRequest) => {
  ...
  if (pgFeeRate !== undefined && (pgFeeRate < 0 || pgFeeRate > 100)) {
    return NextResponse.json(
      { success: false, error: 'PG 수수료율은 0에서 100 사이여야 합니다.' },
      { status: 400 }
    )
  }
